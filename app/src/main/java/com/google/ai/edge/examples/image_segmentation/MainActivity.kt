...
class MainActivity : ComponentActivity() {
  // WakeLock to keep CPU at full performance during inference
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    // Initialize WakeLock for CPU performance
    val powerManager = getSystemService(PowerManager::class.java)
    wakeLock = powerManager.newWakeLock(
      PowerManager.PARTIAL_WAKE_LOCK,
      "ImageSegmentation::InferenceLock"
    )
      ...
      ApplicationTheme {
        BottomSheetScaffold(
          ...
          Content(
            ...
            onTabChanged = {
              tabState = it
              viewModel.stopDepthEstimation()
            },
            onImageProxyAnalyzed = { imageProxy -> viewModel.estimateDepth(imageProxy) },
            onImageBitMapAnalyzed = { bitmap, degrees -> viewModel.estimateDepth(bitmap, degrees) },
          )
        }
      }
    }
  ...
  fun BottomSheet(
    inferenceTime: Long,
    modifier: Modifier = Modifier,
    onDelegateSelected: (DepthHelper.AcceleratorEnum) -> Unit,
    onFlipCamera: () -> Unit,
  ) {
    Column(modifier = modifier.padding(horizontal = 20.dp, vertical = 5.dp)) {
      ...
      OptionMenu(
        ...
        options = DepthHelper.AcceleratorEnum.entries.map { it.name },
      ) {
        onDelegateSelected(DepthHelper.AcceleratorEnum.valueOf(it))
      }
    }
  }
  ...
  override fun onResume() {
    super.onResume()
    // Acquire WakeLock to keep CPU at full performance
    wakeLock.acquire(10 * 60 * 60 * 1000L) // 10 hour timeout as safety
  }
